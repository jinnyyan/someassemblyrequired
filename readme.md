...
By setting another breakpoint on `_run_javascript`, I could extract the C2-generated decrypted JavaScript snippet from memory, as shown below.
...
Could it really be as simple as specifying a big number for the number of blocks mined? Let's find out. By again pausing execution in `_run_javascript`, we can edit the snippet that gets run to harcode a big number for the `blocks_mined` field. This modified snippeet in memory is shown below.
